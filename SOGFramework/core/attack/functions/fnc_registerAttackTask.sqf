 ...
 * 2: Number of targets eliminated to complete the task <NUMBER>
 ...
 * 5: Number of seconds before target(s) escape <NUMBER> (default: nil)
 *
 * Return Value:
 * None
 *
 * Example:
 * ["task1", 1, 2, false] call SOG_attack_fnc_registerAttackTask
 * ["task1", 1, 2, false, false, 45] call SOG_attack_fnc_registerAttackTask
 *
 * Public: Yes
 */
...
if (!isNil "_time") then {
    ...
    [QGVARMAIN(initFramework), {
        ...
        // Check if number of targets to eliminate equal target count
        if (_limitSuccess > count GVAR(allTargets)) then {
            [COMPONENT_STR, "WARNING", format ["Required number of targets to eliminate (set to %1) is higher than the target count (current: %2)", _limitSuccess, count GVAR(allTargets)], true, 0] call EFUNC(main,log);
        };
    ...
    [QGVARMAIN(initFramework), {
        ...
        // Get the target(s) assigned to taskID
        private _targets = GVAR(allTargets) select {GETVAR(_x,GVAR(assignedTask),"") == _taskID};
