...
/*
	...
	["task_name", 1, 2, false, false, 45] call MF_attack_fnc_registerAttackTask
	...
	[QGVARMAIN(initFramework), {
		...
		if (_limitSuccess > count GVAR(allTargets)) then {
			[COMPONENT_STR, "WARNING", format [
				"Required number of targets to eliminate (set to %1) is higher than the target count (current: %2)", _limitSuccess, count GVAR(allTargets)
